...
const GENERATORS = [
	'7d52fba40bd886', '5e8dbf1a03950c', '1c3a3c74072a18', '385d72fa0e5139',
	'7093e5a608865b',
].map(hexToLong)
...
function verifyChecksum(hrp: string, data: number[], enc: EncodingType): boolean {
	const hrpAndData = Array.from(hrpExpand(hrp)).concat(data);
	return polymod(hrpAndData).equals(getEncodingConst(enc));
}

function createChecksum(hrp: string, data: Uint8Array, enc: EncodingType): number[] {
...
export function encode(hrp: string, data: Uint8Array, enc: EncodingType): string {
...
export function decode(blechString: string, enc: EncodingType): { hrp: string, data: Uint8Array } {
	let has_lower = false;
	let has_upper = false;
